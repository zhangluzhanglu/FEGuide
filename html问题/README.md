...
主要分成两部分：也就是浏览器渲染进程（包含了gui线程，js引擎线程，其他线程如ajax,event,定时器等），gui线程解析html+css,js引擎线程执行js,他们是互斥的，页面布局绘制出来后传入GPU进程进行3d绘制，显示。
...
- 在做一个多端通用便签时，遇到一个我问题：如果用户没有点击保存按钮就回到了便签列表，便签列表页面在加载时会自动读取localStorage是否发生了变化，如果变了就保存，词操作在手机上无效，就需要用hidden监听实现
